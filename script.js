...
function migrateFromV11(raw,v){
  const out={...raw};
  if(!v||v<2){
    out.contactReached=Array.isArray(raw.playedGames)&&raw.playedGames.length>=GAME_ORDER.length;
    if(out.contactReached)out.signalUnlocked=true;
    out.schemaVersion=2;
  }
  ...
  if(typeof out.contactReached!=='boolean')out.contactReached=!!out.signalUnlocked;
...
function leaderboardsRail(){return GAME_ORDER.map(id=>{const g=GAMES[id]||{title:id};const b=getBoard(id);const rows=b.length?b.slice(0,5).map((e,i)=>`<div class="lb-row"><span class="lb-rank">${String(i+1).padStart(2,'0')}</span><span class="lb-name">${esc(e.name)}</span><span class="lb-score">${e.score}</span></div>`).join(''):'<p class="story-small lb-empty">NOT PLAYED</p>';return `<div class="lb-board-block"><div class="lb-board-title">${g.title}</div><div class="lb">${rows}</div></div>`}).join('')}
function openLeaderboards(){openModal('LEADERBOARDS','Every score the archive kept.',`<div class="lb-rail">${leaderboardsRail()}</div>`)}
function latchContactReached(){
  if(state.signalUnlocked)return;
  state.contactReached=true;
  state.signalUnlocked=true;
  save();
  if(typeof syncTierBtn==='function')syncTierBtn();
  toast('NEW SIGNAL DETECTED · LOWER LEFT CORNER');
  choiceSound();
}
function openContact(){latchContactReached();openModal('CONTACT','The cleanest way to continue the story.',`<div class="contact-with-lb"><div class="contact-story"><div class="contact-panel"><div class="contact-kicker">OPEN A CHANNEL</div><div class="contact-title">EMAIL RAIYAN</div><p>For work, partnerships, campaigns, growth problems or systems that need fixing.</p><a class="contact-link" href="mailto:raiyang3@gmail.com">raiyang3@gmail.com</a></div><div class="contact-panel wide"><div class="contact-kicker">WHATSAPP</div><div class="contact-title">WHATSAPP RAIYAN</div><p>For quick questions or a faster follow-up on anything you saw here.</p><a class="contact-link" href="https://wa.me/8801988667788" target="_blank" rel="noopener">+8801988667788</a></div><div class="contact-panel alt"><div class="contact-kicker">NEXT CHAPTER</div><div class="contact-title">START A CONVERSATION</div><p>Tell him what needs to grow, what is stuck, or what needs a better operating system.</p><a class="contact-link" href="mailto:raiyang3@gmail.com?subject=Let's%20talk">START TRANSMISSION</a></div></div><div class="lb-rail"><div class="lb-heading">LEADERBOARDS</div>${leaderboardsRail()}</div></div>`);if(ui.lbFloat)ui.lbFloat.classList.remove('hidden')}
