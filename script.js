...
const quests = [
 ['MONSTERCLAW','Affiliate operations across multiple brands and programs.','done'],
 ['TRUELY ESIM','International affiliate recruitment, outreach and partner growth.','done'],
 ['ZAGER GUITARS','Creator operations around 80+ guitar shipments.','done'],
 ['TEAM COMMAND','Managed roughly five affiliate managers for about 1.5 years.','done'],
 ['THE PIPELINE','Built repeatable sourcing, outreach, qualification and activation workflows.','done'],
...
function load(){try{Object.assign(state,JSON.parse(localStorage.getItem('raiyan-ruby-ui-v10')||'{}'))}catch{}}
function current(){return scenes[state.index]}
...
function goto(id){const idx=scenes.findIndex(s=>s.id===id);if(idx<0)return;state.history.push(state.index);state.index=idx;advanceSound();renderScene();save()}
